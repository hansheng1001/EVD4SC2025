...
template <typename T>
void launchKernel_ClearMatrix(dim3 gridDim, dim3 blockDim, long m, long n, T *A, long ldA, cudaStream_t stream)
{
  clearMatrix<<<gridDim, blockDim,0, stream>>>(m, n, A, ldA);
}

template void
launchKernel_ClearMatrix(dim3 gridDim, dim3 blockDim, long m, long n, double *A, long ldA, cudaStream_t stream);

template void
launchKernel_ClearMatrix(dim3 gridDim, dim3 blockDim, long m, long n, float *A, long ldA, cudaStream_t stream);

template void
launchKernel_ClearMatrix(dim3 gridDim, dim3 blockDim, long m, long n, half *A, long ldA, cudaStream_t stream);
...
template <typename T>
void launchKernel_setMetrixTrValue(dim3 gridDim, dim3 blockDim, long m, long n, T *A, long ldA, T v, cudaStream_t stream)
{
  kernel_setMetrixTrValue<<<gridDim, blockDim, 0, stream>>>(m, n, A, ldA, v);
}
...
template <typename T>
void launch_kernel_cpyATr2Vector(dim3 gridDim, dim3 blockDim, long m, long n, T *A, long ldA, T *B, cudaStream_t stream)
{
  kernel_cpyATr2Vector<<<gridDim, blockDim, 0, stream>>>(m, n, A, ldA, B);
}
...
template <typename T>
__global__ void kernel_cpyVector2ATr(long m, long n, T *A, long ldA, T *B)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename T>
void launch_kernel_cpyVector2ATr(dim3 gridDim, dim3 blockDim, long m, long n, T *A, long ldA, T *B, cudaStream_t stream)
{
  kernel_cpyVector2ATr<<<gridDim, blockDim, 0, stream>>>(m, n, A, ldA, B);
}
template void launch_kernel_cpyVector2ATr(dim3 gridDim,
                                          dim3 blockDim,
                                          long m,
                                          long n,
                                          double *A,
