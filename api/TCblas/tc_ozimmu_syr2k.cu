...
__global__ void setInitialValueDouble(long int m, long int n, double *a, long int lda, double val)
{
...
__global__ void matrixCpyDouble(long int m, long int n, double *a, long int lda, double *b, long int ldb)
{
    long int i = threadIdx.x + blockDim.x * blockIdx.x;
    long int j = threadIdx.y + blockDim.y * blockIdx.y;
    if (i < m && j < n)
...
void tc_ozimmu_syr2k_p2(cublasHandle_t handle, long int n, long int k, double alpha, double *A, long int lda, double *B, long int ldb, double beta, double *C, long int ldc, long int nb)
{
    cublasGemmStridedBatchedEx(handle, CUBLAS_OP_N, CUBLAS_OP_T,
                               ...
                               A, CUDA_R_64F, lda, nb,
                               B, CUDA_R_64F, ldb, nb,
                               &beta, C, CUDA_R_64F, ldc, nb + nb * ldc,
                               n / nb, CUDA_R_64F, CUBLAS_GEMM_DEFAULT_TENSOR_OP);
    cublasGemmStridedBatchedEx(handle, CUBLAS_OP_N, CUBLAS_OP_T,
                               nb, nb, k, &alpha,
                               B, CUDA_R_64F, ldb, nb,
                               A, CUDA_R_64F, lda, nb,
                               &done, C, CUDA_R_64F, ldc, nb + nb * ldc,
...
void tc_ozimmu_syr2k_p3(cublasHandle_t handle, long int n, long int k, double alpha, double *A, long int lda, double *B, long int ldb, double beta, double *C, long int ldc, long int nb)
{
    ...
    for (int i = length; i >= 0; i--)
    {
        ...
        else
        {
            ...
            cublasDsyr2k(handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, nn, k, &alpha, A + offset, lda,
                         B + offset, ldb, &beta, C + offset + offset * ldc, ldc);
